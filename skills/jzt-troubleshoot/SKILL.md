...
组合 `skywalking`（MCP） + `elk`（**CLI**） + `jztsql`（MCP） + `sepp`（MCP） 定位线上问题根因。四者相互独立，按下面的顺序与桥接点组合使用。

> 日志一律走 `elk` 命令行（stdout 为 JSON），不用 MCP 工具；`--env` 是 search/trace 的**必填**参数。

## 总体流程

```
1. 明确范围   服务/项目 + 环境(dev/test/pre/prod/stg) + 时间窗 + 现象（接口 URL / 报错信息 / TraceId）
2. 链路入手   skywalking.analyze_endpoint  → 性能汇总、错误率、最慢链路、耗时热点
3. 日志佐证   elk CLI（list-projects → search → trace → expand）拿具体报错与堆栈
...
各 CLI / MCP 的完整参数说明、返回结构与避坑清单在 `references/` 目录，**执行到对应步骤前再 Read 对应文件**：

| 文件 | 内容 | 读取时机 |
|---|---|---|
| `references/skywalking-trace.md` | 6 个工具参数、时间参数约定、Entry 端点/时区坑、空档(gaps)分析 | 第 2 步前 |
| `references/elk-logs.md` | elk CLI 工作流、`--env` 取值、query 引号规则、summary→expand | 第 3 步前 |
| `references/reference/cli.md` | elk 各子命令完整参数与多集群差异 | CLI 参数拿不准时 |
| `references/jztsql.md` | 只读约束、实例/库默认值、limit_num | 第 4 步前 |
...
- **环境**：`elk --env` 必填，取值 dev / test / pre（共享非生产 ES）、prod / stg（独立生产 os-elk）。用户没说就问一次，不要猜
...
- 拿到 `slowest_traces` 中某条 `trace_id` 后：`analyze_trace(trace_id=...)`
  ...
  - 看 `gaps` / `gap_stats`：父 span 内未被子 span 覆盖的空档 → 未埋点/本地处理/等待
  - 看 `errors`：错误 span 直接给根因线索
- 链路默认只保留约 7 天：查更早窗口时 `total` 偏小未必是"没被调用过"，响应会带保留期 `note`
- 注意：端点指标只在 Entry 端点所在服务上产生，`analyze_endpoint` 已内置解析，优先用它而不是手工查指标。

## 第 3 步：ELK 日志佐证（elk CLI）

**桥接点：SkyWalking 的 `trace_id` ≈ ELK 日志里的 TID**

```bash
elk list-projects                                    # 确定 project 名与所属 env
# 有 TID：
elk trace '<TID>' --env test --project zs-saas-crm-admin --from now-6h
# 无 TID：关键词摘要检索（默认 summary）
elk search '"queryReceptionDocumentDetail"' --env test --project zs-saas-crm-admin --from now-6h --size 20
# 只展开关键 hit，且 -o 落盘后再 Read
elk expand <hit._index> <hit._id> --env test -o /tmp/hit.json
```

- `--env` 必填，没给环境先问用户；`--project` 强烈建议带上（生产尤其）
- 驼峰 / 接口路径要加双引号：`'"queryReceptionDocumentDetail"'`，否则分词 0 命中
- 默认 `summary` 模式；完整堆栈只对 1~3 条关键 ERROR 用 `expand -o` 落盘，禁止把全文打 stdout
- `total=0` 时读返回 `meta` / `hints`（实际 query、索引、时间窗）再改查询；`search` 默认 `now-1h`，更早要放宽 `--from`
...
- `limit_num` 默认 100（上限 20000）；默认实例 `生产-诊所-读库-ALI`（生产读库），跨实例先 `list_instances` / `list_databases`
...
1. 时间窗不一致导致"链路有、日志没有"：链路与日志用同一时间窗再查一次
2. SkyWalking 指标全 0：时区问题（`SKYWALKING_TZ` 与 OAP 不一致）
3. elk 搜不到驼峰/路径：没加双引号被分词；`search` 默认 `now-1h` 也会漏掉更早数据
4. jztsql 查生产读库有延迟，刚写入的数据可能查不到
5. 别跳过第 1 步直接查：没有 project+env 会跨候选项目聚合，噪音大
6. `elk` 命令找不到 / 报错：在 `/Users/zhengzihang/Documents/my-mcp/elk-mcp-server` 执行
   `.venv/bin/python -m pip install -e .` 重装；错误是 JSON 且退出码非 0
...
3. 产出了比本文档模板更优的排查路径或结论结构
4. CLI/MCP 实际行为与 `references/` 文档不符（参数、返回结构、默认值）
